...
* @return RRT Tree
*/
vector<RRT::rrtNode> RRT::getTree() const 
{
    return rrtTree;
}
...
vector<int> RRT::getRootToEndPath(int endNodeID)
{
    ...
    // while(rrtTree[path.back()].nodeID != 0)
    while(path.back() != 0)
    {
        //std::cout<<rrtTree[path.back()].nodeID<<endl;
        //path.insert(path.begin(),rrtTree[path.front()].parentID);//**
        path.push_back(rrtTree[path.back()].parentID);
    }
