...
void initializeMarkers(visualization_msgs::Marker &sourcePoint,
  ...
  // defining types
  rrtTreeMarker.type = visualization_msgs::Marker::LINE_LIST; //线条序列
  finalPath.type = visualization_msgs::Marker::LINE_STRIP;    //线条
  sourcePoint.type = goalPoint.type = randomPoint.type = visualization_msgs::Marker::SPHERE; //球体
  ...
  sourcePoint.color.g = 0.0f;
  sourcePoint.color.b = 0.0f;
  goalPoint.color.r = 0.0f;
  goalPoint.color.g = 1.0f;
  goalPoint.color.b = 0.0f;
  randomPoint.color.r = 0.0f;
  randomPoint.color.g = 0.0f;
...
double judge_dis_to_goal(vector<vector<geometry_msgs::Point> >& ObsArr,int X, int Y){
    double dis = sqrt(pow(ObsArr[0][0].x-X,2)+pow(ObsArr[0][0].y-Y,2));
    for(int i=0; i<ObsArr.size(); i++){
        for(int j=0; j<ObsArr[i].size(); j++){
            dis = min(dis,sqrt(pow(ObsArr[i][j].x-X,2)+pow(ObsArr[i][j].y-Y,2)));
        }
    }
    std::cout<<"Obstacles points min_dis to goal is "<<dis<<endl;
    return dis;
}

void addBranchtoRRTTree(visualization_msgs::Marker &rrtTreeMarker,
                        RRT::rrtNode &tempNode, RRT &myRRT) //添加分支到RRT_tree
{
...
bool HaveMixed(geometry_msgs::Point &first, geometry_msgs::Point &second,
               RRT::rrtNode &tempNode) {
  if ((tempNode.posY < first.y && tempNode.posY < second.y) ||
      (tempNode.posY > first.y && tempNode.posY > second.y))
    return false;
  else
    return true;
}
...
bool checkIfOutsideObstacles(vector<vector<geometry_msgs::Point> > &obstArray,
  ...
  //     AMAB = (((tempNode.posX - obstArray[i][0].x) * (obstArray[i][1].x -
  //     obstArray[i][0].x)) + (( tempNode.posY - obstArray[i][0].y) *
  //     (obstArray[i][1].y - obstArray[i][0].y)));
  //     AMAD = (((tempNode.posX - obstArray[i][0].x) * (obstArray[i][3].x -
  //     obstArray[i][0].x)) + (( tempNode.posY - obstArray[i][0].y) *
  ...
  for (int i = 0; i < obstArray.size(); i++) {
    ...
    for (int j = 0; j < obstArray[i].size() - 1; j++) {
      if (HaveMixed(obstArray[i][j], obstArray[i][j + 1], tempNode)) {
        temp = obstArray[i][j + 1].x -
               ((obstArray[i][j + 1].x - obstArray[i][j].x) *
                (obstArray[i][j + 1].y - tempNode.posY) /
                (obstArray[i][j + 1].y - obstArray[i][j].y));
        if (temp < tempNode.posX)
          left++;
        else if (temp > tempNode.posX)
          right++;
        else
          return false;
      }
...
bool addNewPointtoRRT(RRT &myRRT, RRT::rrtNode &tempNode, float rrtStepSize,
...
bool checkNodetoGoal(int X, int Y, RRT::rrtNode &tempNode,double dis_to_goal) {
  double distance = sqrt(pow(X - tempNode.posX, 2) + pow(Y - tempNode.posY, 2));
  if (distance < dis_to_goal) {
    return true;
  }
  return false;
}
...
int main(int argc, char **argv) {
  ...
  ros::Publisher rrt_publisher =
      n.advertise<visualization_msgs::Marker>("path_planner_rrt", 100);
  ...
  double dis_to_goal;
  dis_to_goal = judge_dis_to_goal(obstacleList,goalX,goalY);
  ...
  while (ros::ok() && status) {
    if (rrtPaths.size() < rrtPathLimit) {
      ...
      if (addNodeResult) {
        ...
        nodeToGoal = checkNodetoGoal(goalX, goalY, tempNode,dis_to_goal);
        if (nodeToGoal) {
          path = myRRT.getRootToEndPath(tempNode.nodeID);
          rrtPaths.push_back(path);
          std::cout << "New Path Found. Total paths " << rrtPaths.size()
                    << endl;
    ...
    } else // if(rrtPaths.size() >= rrtPathLimit)
    {
      ...
      std::cout << "Optimal Path custom : " << shortestPathLength + dis_to_goal << endl;
